...
function normalizeBallSpeed() {
  ...
  if (currentSpeed > 0) {
    gameState.ball.dx =
      (gameState.ball.dx / currentSpeed) * gameState.ball.baseSpeed;
    gameState.ball.dy =
      (gameState.ball.dy / currentSpeed) * gameState.ball.baseSpeed;
  }
}
...
function accelerateBall() {
  ...
  const newSpeed = Math.min(
    currentSpeed * gameState.ball.acceleration,
    gameState.ball.maxSpeed
  );
...
function updateGame() {
  if (!gameState.gameActive || Object.keys(gameState.players).length < 2)
    return;
  ...
  socket.on("paddleMove", (data) => {
    ...
    if (playerRole && gameState.paddles[playerRole]) {
      ...
      if (data.y !== undefined) {
        paddle.y = Math.max(
          0,
          Math.min(gameState.canvas.height - paddle.height, data.y)
        );
      }
      ...
      if (data.x !== undefined) {
        if (playerRole === "player1") {
          ...
          paddle.x = Math.max(
            10,
            Math.min(gameState.canvas.width / 2 - paddle.width - 50, data.x)
          );
        } else {
          // Player 2 can move in right half of screen
          paddle.x = Math.max(
            gameState.canvas.width / 2 + 50,
            Math.min(gameState.canvas.width - paddle.width - 10, data.x)
          );
        }
      }
    }
